...
from utils import norm_denoise
...
class Trainer():
  ...
  """
  def __init__(self,  dataset, net, loss_f, metric, 
              learning_rate=1e-3, 
    ...
    with torch.no_grad():
        for batch_X, target in test:
          ...
          mean_metric += self.metric(Y_pred,
                        target.to(self.device)).mean().item()
          batch_n += 1
  ...
  def fit(self, data_train, data_val):
      ...
      optimizer = self.optim(model.parameters(), lr=self.learning_rate)

      if self.scheduler is not None:
          scheduler = self.scheduler(optimizer)

      train = self.dataset(data_train)
      val = self.dataset(data_val)
      ...
      for epoch in range(self.epoch_amount): 
          ...
          for batch_X, target in train:
              ...
              mean_loss += float(loss)
              mean_metric += float(self.metric(Y_pred, target))
              batch_n += 1
